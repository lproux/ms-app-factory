...
export interface CopilotStudioCombinedJudgeOptions {
  personas: Persona[];
  directLineSecret?: string;
  userId?: string;
  baseUrl?: string;
...
/**
 * A Copilot Studio (Direct Line) judge that scores ALL personas in one
 * conversational round-trip. Used by the `compact` panel shape.
 */
export function makeCopilotStudioCombinedJudge(
  opts: CopilotStudioCombinedJudgeOptions,
): CombinedJudge {
  const personas = opts.personas;
  const id = 'cs:combined';
  const baseUrl = (opts.baseUrl ?? DEFAULT_BASE_URL).replace(/\/+$/, '');
  const userId = opts.userId ?? `app-factory-judge-combined`;
  ...
  return {
    ...
    async review(artifact: JudgeArtifact): Promise<Verdict[]> {
      ...
      if (!secret) {
        throw new AppFactoryError(
          'CS_JUDGE_NO_SECRET',
          'Copilot Studio combined judge requires a Direct Line secret (opts.directLineSecret or COPILOT_STUDIO_JUDGE_SECRET)',
          { recoverable: true, details: { judge: id } },
        );
      }
      ...
      log.debug({ judge: id, conversationId }, 'opened direct line conversation (combined)');
      ...
      if (!lastBotText) {
        throw new AppFactoryError(
          'CS_JUDGE_TIMEOUT',
          `Copilot Studio combined judge produced no reply within ${POLL_BUDGET_MS}ms`,
          { recoverable: true, details: { judge: id } },
        );
      }

      return parseCombinedVerdicts(lastBotText, id, personas);
    },
  };
}
